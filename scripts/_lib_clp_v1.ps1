...
function _JsonEscape([string]$s){
  ...
  while($i -lt $s.Length){
    ...
    elseif($c -eq 12){  [void]$sb.Append([char]92); [void]$sb.Append([char]102) }    # \\f
    elseif($c -eq 10){  [void]$sb.Append([char]92); [void]$sb.Append([char]110) }    # \\n
    elseif($c -eq 13){  [void]$sb.Append([char]92); [void]$sb.Append([char]114) }    # \\r
    elseif($c -eq 9){   [void]$sb.Append([char]92); [void]$sb.Append([char]116) }    # \\t
    elseif($c -lt 32){
      [void]$sb.Append([char]92); [void]$sb.Append([char]117) # \\u
      $hex = ("{0:x4}" -f $c)
      [void]$sb.Append($hex)
...
function ReadJson([string]$Path){ if(-not (Test-Path -LiteralPath $Path -PathType Leaf)){ Die ("MISSING_JSON: " + $Path) }; $raw=[System.IO.File]::ReadAllText($Path,(New-Object System.Text.UTF8Encoding($false))); return ($raw | ConvertFrom-Json) }
